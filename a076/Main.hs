...
main = do
  [n] <- getLineIntList
  xs <- getLineIntVec
  let csums = VU.scanl' (+) 0 (VU.generate (2 * n) (\i -> xs VU.! (i `rem` n)))
  let goal = csums VU.! n `div` 10

  let search (start, end) =
        -- FIXME: why do we need `min`..?
        fst $ bsearch (end, min (2 * n) (end + n)) (\i -> csums VU.! i - csums VU.! start <= goal)

  let solve = fst $ foldl' step s0 [0 .. n]
      s0 = (False, 1)
      step st@(True, _) _ = st
      step (False, e) start =
        let end = max e (succ start)
         in case search (start, end) of
              Nothing -> (False, end)
              Just end' ->
                let b = csums VU.! end' - csums VU.! start == goal
                 in (b, end')

  putStrLn $ if solve then "Yes" else "No"
