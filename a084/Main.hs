...
main = do
  [n] <- getLineIntList
  xs <- VU.fromList @Int . map (\case 'x' -> 1; 'o' -> 2; _ -> error "") <$> getLine

  -- TODO: refactor
  let nextR l r0 =
        let r = succ l `max` r0
         in find (\i -> xs VU.! l .|. xs VU.! i == 3) [r .. n - 1]

  let result = mapAccumL step (1, 1) [0 .. n - 2]
      step (r1, r2) l =
        case xs VU.! l of
          1 -> case nextR l r2 of
            Just r ->
              -- let !_ = traceShow ((l, r), n - r) () in
              ((r1, r), n - r)
            Nothing -> ((r1, n - 1), 0)
          2 -> case nextR l r1 of
            Just r ->
              -- let !_ = traceShow ((l, r), n - r) () in
              ((r, r2), n - r)
            Nothing -> ((n - 1, r2), 0)
