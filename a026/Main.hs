...
-- {-# INLINE dfs #-}
dfs :: Graph -> (s -> Int -> Int -> Graph -> s) -> s -> Int -> s
dfs graph f s0 start =
  let (_, s, _) = step (IS.empty, s0, 0) start
   in s
  where
    step (explored, s, depth) x =
      -- NOTE: `x` is always unexplored
      let s' = f s depth x graph
          neighbors = filter (not . (`IS.member` explored)) $ graph ! x
          -- !_ = trace (show (x, depth, neighbors)) ""
          explored' = IS.insert x $ explored `IS.union` IS.fromList neighbors
          -- FIXME:
          (ee, ss, _) = foldl' step (explored', s', succ depth) neighbors
       in (ee, ss, depth)

main :: IO ()
main = do
  [n] <- getLineIntList
  input <- concatMap (\[a, b] -> [(a, b), (b, a)]) <$> replicateM (n - 1) getLineIntList
  let graph = accumArray (flip (:)) [] (1, n) input

  let (evens, odds) = bimap VU.fromList VU.fromList $ dfs graph visit ([], []) 1
      visit = \(xs, ys) d x _ -> if even d then (x : xs, ys) else (xs, x : ys)

  let targets =
        VU.take (n `div` 2) $
          if VU.length evens >= VU.length odds
            then evens
            else odds

  VU.forM_ targets $ \x -> do
    print x
