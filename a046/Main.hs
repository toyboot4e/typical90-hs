...
main = do
  [n] <- getLineIntList
  xs1 <- accumArray @UArray (+) (0 :: Int) (0, 45) . map ((,1) . (`rem` 46)) <$> getLineIntList
  xs2 <- accumArray @UArray (+) (0 :: Int) (0, 45) . map ((,1) . (`rem` 46)) <$> getLineIntList
  xs3 <- accumArray @UArray (+) (0 :: Int) (0, 45) . map ((,1) . (`rem` 46)) <$> getLineIntList

  let count =
        sum
          [ (xs1 ! i1) * (xs2 ! i2) * (xs3 ! i3)
            | i1 <- [0 .. 45],
              i2 <- [0 .. 45],
              i3 <- [0 .. 45],
              (i1 + i2 + i3) `rem` 46 == 0
