...
main = do
  ...
  -- array of (input characters, filled characters)
  let dp =
        array
          ((0, 0), (n, 7))
          $ [((0, j), 0) | j <- [0 .. 7]]
            ++ [ ((i, j), result)
                 | i <- [1 .. n],
                   j <- [0 .. 7],
                   -- NOTE: (i, j) is the DP index
                   let onKeep = dp ! (i - 1, j),
                   let onGet =
                         case (j, BS.index s (i - 1)) of
                           (1, 'a') -> 1
                           (2, 't') -> dp ! (i - 1, j - 1)
                           (3, 'c') -> dp ! (i - 1, j - 1)
                           (4, 'o') -> dp ! (i - 1, j - 1)
                           ...
                           (7, 'r') -> dp ! (i - 1, j - 1)
                           _ -> 0,
                   let result = (onKeep + onGet) `rem` unit
               ]
...
-- Map-based function memoization:
-- https://toslunar.hatenablog.com/entry/20100408/1270719176
      ...
      dp = memoize $ \(nRead, nFilled) -> do
        if nRead == 0
          then return 0
            ...
            onKeep <- dp (nRead - 1, nFilled)

            let lastCh = BS.index s (nRead - 1)
            onGet <- case (nFilled - 1, lastCh) of
              (0, 'a') -> return 1
              (1, 't') -> dp (nRead - 1, nFilled - 1)
              (2, 'c') -> dp (nRead - 1, nFilled - 1)
              (3, 'o') -> dp (nRead - 1, nFilled - 1)
              (4, 'd') -> dp (nRead - 1, nFilled - 1)
              (5, 'e') -> dp (nRead - 1, nFilled - 1)
              (6, 'r') -> dp (nRead - 1, nFilled - 1)
              _ -> return 0

            return $ (onKeep + onGet) `rem` unit
