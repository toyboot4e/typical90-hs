...
main = do
  [t'] <- getLineIntList
  [l', x', y'] <- getLineIntList
  [q] <- getLineIntList
  es <- concat <$> replicateM q getLineIntList

  let t = fromIntegral t' :: Double
  let l = fromIntegral l' :: Double
  let x = fromIntegral x' :: Double
  let y = fromIntegral y' :: Double

  -- let center = (0.0, 0.0, l / 2.0)

  forM_ es $ \e' -> do
    let e = fromIntegral e' :: Double
    let theta = 2 * pi * e / t

    let a = (x, y, 0.0)
    let pos = (0.0, l / 2.0 * (- sin theta), l / 2.0 * (- cos theta) + l / 2.0)
    let ap = opVec (-) a pos

    let apOnZ = case ap of
          (x1, x2, _) -> (x1, x2, 0.0)
    let phi = acos $ vecLen apOnZ / vecLen ap

    -- print (theta, pos, ap, apOnZ)
    print $ phi * 180.0 / pi

type Vec = (Double, Double, Double)

vecLen :: Vec -> Double
vecLen (x, y, z) = sqrt $ x * x + y * y + z * z

opVec :: (Double -> Double -> Double) -> (Double, Double, Double) -> (Double, Double, Double) -> (Double, Double, Double)
opVec op (x, y, z) (x', y', z') = (x `op` x', y `op` y', z `op` z')
