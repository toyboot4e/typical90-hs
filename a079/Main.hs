...
hashArray :: [Int] -> Int
hashArray = foldl' (\x y -> x * y `rem` modulo) 1
  where
    modulo = 10 ^ 9 + 7
...
main = do
  ...
  -- [(offset, diffArrayHash)]
  -- if the multiset matches, count minimum offset diffs

  -- TODO: calculate the number
  (_, xs) <- fromListMS . map hashArray <$> replicateM h getLineIntList
  (_, ys) <- fromListMS . map hashArray <$> replicateM h getLineIntList
  print "TODO"
