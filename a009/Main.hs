...
main = do
  [n] <- getLineIntList
  cps <- VU.replicateM n ((\[c, p] -> (c, p)) <$> getLineIntList)

  let step = \(c, p) (s1, s2) -> if c == 1 then (p + s1, s2) else (s1, p + s2)
  let csums = VU.scanl' step (0, 0) cps

  [q] <- getLineIntList
  qs <- replicateM q ((\[c, p] -> (c, p)) <$> getLineIntList)

  let bs =
        flip map qs $ \(l, r) ->
          let s1 = fst (csums VU.! r) - fst (csums VU.! (l - 1))
              s2 = snd (csums VU.! r) - snd (csums VU.! (l - 1))
           in mconcat (intersperse (BSB.charUtf8 ' ') $ map BSB.intDec [s1, s2]) <> BSB.charUtf8 '\n'

  BSB.hPutBuilder stdout $ mconcat bs
