...
-- We have to stich with `Integer` to pass the fourth sample test? It's tough.
main :: IO ()
main = do
  input <- getLineIntList
  let [l, r] = map fromIntegral input :: [Integer]

  let div2 = fromIntegral $ invModF 2 (fromInteger modulus) :: Integer

  let count = foldl' (\acc x -> (acc + x) `rem` modulus) 0 $ map f [0 .. 18]
      f n =
        let x = 10 ^ n
            l' = max x l
            s = (l' + r) `rem` modulus * (r - l' + 1) `rem` modulus * div2 `rem` modulus
         in if l' > r
              then 0
              else
                let !_ = traceShow (n, (l', r), s) ()
                 in s
