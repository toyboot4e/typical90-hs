...
main = do
  ...
  let results = reverse . snd $ foldl' step s0 queries
      s0 = (Seq.empty, [])
      step (seq_, acc) [1, x] = (x Seq.<| seq_, acc)
      step (seq_, acc) [2, x] = (seq_ Seq.|> x, acc)
      -- TODO: isn't it too slow?
      step (seq_, acc) [3, i] = (seq_, Seq.index seq_ (pred i) : acc)
      step _ _ = error "unrechable"
