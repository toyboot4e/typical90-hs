...
-- -- gcdN = (foldl1' gcd) . sort

-- -- | Least common measure
-- lcm :: (Integer, Integer) -> Integer
-- lcm (a, b) = a * b `div` gcd (a, b)

-- TODO: mod calculation
...
main = do
  [n] <- getLineIntList
  [a, b, c] <- reverse . sort <$> getLineIntList

  let abc = foldl1' lcm [a, b, c]
  let (nAbc, n') = n `divMod` abc

  let x =
        -- REMARK: `head` results in WA while `minimum` works
        -- WHY???
        minimum
          [ ia + ib + ic
            | let na = n' `div` a,
