...
dfs graph start = loop (-1, 0) [] start
  where
    loop :: (Int, Int) -> [(Int, Int)] -> Int -> [(Int, Int)]
    loop (lastX, d) visits x =
      let visits' = (x, d) : visits
          d' = succ d
       in foldl' (loop (x, d')) visits' (filter (/= lastX) $ graph ! x)

main :: IO ()
main = do
  [n] <- getLineIntList

  -- REMARK: THE GRAPH IS CONNECTED as stated in the problem.
  -- REMARK: IT'S A TREE and THERE'S NO LOOP.
  edges <- concatMap (\[a, b] -> [(a, b), (b, a)]) <$> replicateM (n - 1) getLineIntList
  let graph = accumArray (flip (:)) [] (1, n) edges

  let (a, _) = (maximumBy . comparing) snd $ dfs graph 1
  let (_, d) = (maximumBy . comparing) snd $ dfs graph a

  -- print $ (maximumBy . comparing) snd $ dfs graph 1
  -- print $ (maximumBy . comparing) snd $ dfs graph a
