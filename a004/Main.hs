...
main = do
  ...
  let matrix = listArray ((1, 1), (h, w)) (concat input) :: Array (Int, Int) Int

  let rowSums = VU.generate h (\y -> sum $ map (\x -> matrix ! (y + 1, x + 1)) [0 .. (w - 1)])
  let colSums = VU.generate w (\x -> sum $ map (\y -> matrix ! (y + 1, x + 1)) [0 .. (h - 1)])

  forM_ [1 .. h] $ \y -> do
    let s = flip map [1 .. w] $ \x ->
          show $ rowSums VU.! (y - 1) + colSums VU.! (x - 1) - matrix ! (y, x)
    putStrLn $ unwords s
