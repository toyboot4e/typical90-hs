...
main :: IO ()
main = do
  [n, p, q] <- getLineIntList
  xs <- getLineIntVec

  let mulMod x y = x * y `rem` p
  ...
  let result = length
        [ True
          | i1 <- [0 .. n - 5],
            ...
            i4 <- [succ i3 .. n - 2],
            i5 <- [succ i4 .. n - 1],
            -- let remainder = foldl1' mulMod $ map (xs VU.!) [i1, i2, i3, i4, i5],
            let remainder = (xs VU.! i1) `mulMod` (xs VU.! i2) `mulMod` (xs VU.! i3) `mulMod` (xs VU.! i4) `mulMod` (xs VU.! i5),
            remainder == q
        ]
