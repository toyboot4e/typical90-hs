...
main :: IO ()
main = do
  [nSeq, constraint] <- getLineIntList
  xs <- getLineIntVec

  let advanceR :: MultiSet -> (Int, Int) -> (MultiSet, (Int, Int))
      advanceR ms (l, r)
        | r == nSeq - 1 = (ms, (l, r))
        | otherwise =
          let r' = succ r
              ms'@(n', _) = incrementMS (xs VU.! r') ms
           in -- !_ = traceShow ((l, r), fst ms, (l, r'), n', n' <= constraint) ()
              if n' <= constraint
                then advanceR ms' (l, r')
                else (ms, (l, r))

  let advanceL :: MultiSet -> (Int, Int) -> (MultiSet, (Int, Int))
      advanceL ms (l, r)
        | l == r = (singletonMS (xs VU.! succ l), (succ l, succ r))
        | l < r =
          let ms' = decrementMS (xs VU.! l) ms
           in (ms', (succ l, r))
        | otherwise = error "unreachable"

  let result = scanl' step s0 [1 .. nSeq - 1]
      s0 = advanceR (singletonMS (xs VU.! 0)) (0, 0)
      step (ms, (l, r)) _ =
        let (ms', (l', r')) = advanceL ms (l, r)
         in advanceR ms' (l', r')

  -- let !_ = traceShow (map snd result) ()
  print $ maximum $ map (\(_, (l, r)) -> r - l + 1) result
