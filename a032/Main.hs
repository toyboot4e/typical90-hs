...
main = do
  ...
  -- let !_ = traceShow players ()
  -- let !_ = traceShow banned ()

  -- wow, ridiculous code..
  let result = minimum $ map (fst . foldl' f s0) ps
      ps :: [[(Int, Int)]]
      ps = map (zip [0 :: Int ..]) (permutations [0 :: Int .. pred n])
      s0 = (0 :: Int, maxBound @Int)
      send = (maxBound @Int, maxBound @Int)
  ...
  print $ if result == maxBound @Int then (-1) else result + n
