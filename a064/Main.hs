...
main = do
  [n, q] <- getLineIntList
  initialHeights <- getLineIntList
  queries <- replicateM q (map pred <$> getLineIntList)

  let ((cost0, _), diffs0) = mapAccumL accStep (0, 0) initialHeights
      accStep (cost, lastX) x = ((cost + abs (x - lastX), x), x - lastX)

  diffs <- VU.unsafeThaw $ VU.fromList diffs0

  -- theres no scanM, so
  lastCost <- newIORef cost0

  forM_ queries $ \[l, r, v] -> do
    diffL <- if inRange (0, n - 1) l then (\d -> abs d - abs (d + v)) <$> VUM.read diffs l else return 0
    diffR <- if inRange (0, n - 1) r then (\d -> abs d - abs (d + v)) <$> VUM.read diffs r else return 0
    VUM.modify diffs (+ v) l
    VUM.modify diffs (+ v) r

    cost <- readIORef lastCost
    let newCost = cost + diffL + diffR
    writeIORef lastCost newCost
