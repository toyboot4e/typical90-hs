...
main = do
  ...
  xs' <- getLineIntVec
  [q] <- getLineIntList
  bs <- VU.fromList . concat <$> replicateM q getLineIntList

  let xs = VU.modify VAI.sort xs'

  let toCost = \x ->
        case bsearch (0, n - 1) (\i -> xs VU.! i < x) of
          (Just l, Just r) -> abs (xs VU.! l - x) `min` abs (xs VU.! r - x)
          (Just l, Nothing) -> abs (xs VU.! l - x)
          (Nothing, Just r) -> abs (xs VU.! r - x)
          (Nothing, Nothing) -> error "unreachable"

  VU.mapM_ (print . toCost) bs
