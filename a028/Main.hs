...
main = do
  ...
  let toSeeds = \[x1, y1, x2, y2] ->
        -- REMARK: we're conuting areas, not dots
        [ ((y1, x1), 1),
          ((y1, x2), -1),
          ((y2, x1), -1),
          ((y2, x2), 1)
  ...
  input <- concatMap toSeeds <$> replicateM n getLineIntList
  let bounds_ = ((0, 0), (h + 1, w + 1))
  let seeds = accumArray (+) (0 :: Int) bounds_ input
  let mat = ismo2D bounds_ seeds

  -- let !_ = traceMat2D seeds (0, h) (0, w)
  -- let !_ = trace "--------" ()
  -- let !_ = traceMat2D mat (0, h) (0, w)

  let counter = VU.create $ do
        let nn = 1 + 10 ^ 5 :: Int
        vec <- VUM.replicate nn (0 :: Int)

        forM_ (range ((0, 0), (w, h))) $ \(y, x) -> do
          let v = mat ! (y, x)
          -- let !_ = trace (show ((y, x), v)) ""
          VUM.modify vec (+ 1) v
