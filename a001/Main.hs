...
main = do
  ...
  let xs =
        evalState
          (VU.mapM (\x -> state $ \last' -> (x - last', x)) xs')
          (0 :: Int)

  let lastX = l - VU.last xs'

  let concludedSpanLen = fromJust . fst $ bsearch (1, l) (\minSpanLen -> countSpans minSpanLen xs lastX >= (k + 1))
  -- let concludedSpanLen = fromJust . snd $ bsearch (1, l) (\minSpanLen -> countSpans minSpanLen xs lastX > (k + 1))
  -- print concludedSpanLen
  print $ calcMinSpan concludedSpanLen xs lastX

-- TODO: also return the minimim length
countSpans :: Int -> VU.Vector Int -> Int -> Int
countSpans minSpanLen xs lastX =
  snd $ VU.foldl' step (0 :: Int, 0 :: Int) xs `step` lastX
  where
    step (len, nSpans) x =
      let x' = len + x
          -- !_ = trace (show minSpanLen ++ ": x = " ++ show x ++ ", (len, nSpans) = " ++ show (len, nSpans)) ()
       in if x' >= minSpanLen then (0, nSpans + 1) else (x', nSpans)

-- TODO: also return the minimim length
calcMinSpan :: Int -> VU.Vector Int -> Int -> Int
calcMinSpan minSpanLen xs lastX =
  snd $ VU.foldl' step (0 :: Int, maxBound :: Int) xs `step` lastX
  where
    step (len, minSpan) x =
      let x' = len + x
       in if x' >= minSpanLen then (0, minSpan `min` x') else (x', minSpan)
