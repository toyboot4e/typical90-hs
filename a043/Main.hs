...
main = do
  ...
  end <- (\[a, b] -> (a, b)) <$> getLineIntList

  input <- concatMap (map (== '#')) <$> replicateM h getLine
  let blocks = listArray @UArray bounds_ input

  let go :: (Int, Int) -> (Int, Int) -> (Int, Int)
      go (yy, xx) (dy, dx) = loop (yy, xx)
        where
          loop (y, x) =
            let yx' = (y + dy, x + dx)
             in if inRange bounds_ yx' && not (blocks ! yx')
                  then loop yx'
                  else (y, x)

  explored <- newArray @IOUArray bounds_ False
  writeArray explored start True
  let dirs = [(0, 1), (0, -1), (1, 0), (-1, 0)] :: [(Int, Int)]

  -- FIXME: Much better monadic (imperative) loop? (Maybe use IntMap instead of MArray?)
  result <- flip unfoldM (0 :: Int, [start]) $ \(n, ps) -> do
    if null ps
      -- TODO: early return?
      then return Nothing
      else do
        ps' <- forM ps $ \p -> do
          p' <- forM dirs $ \dir -> do
            ...
            b <- readArray explored pos
            if b
              then return Nothing
              else do
                writeArray explored pos True
                return $ Just pos

          return $ catMaybes p'

        b <- readArray explored end
        return $ if b then Nothing else Just (n + 1, (n + 1, concat ps'))

  if null result
    then print (0 :: Int)
    else print $ last result
