...
main :: IO ()
main = do
  [nXs, nQueries] <- getLineIntList
  xs <- getLineIntVec >>= VU.thaw
  queries <- replicateM nQueries getLineIntList

  baseI <- newIORef (0 :: Int)

  let getIndex i = do
        base <- readIORef baseI
        return $ (base + i) `rem` nXs

  forM_ queries $ \case
    [1, i1, i2] -> do
      j1 <- getIndex $ pred i1
      j2 <- getIndex $ pred i2
      -- let !_ = traceShow ("swap", j1, j2) ()
      VUM.swap xs j1 j2
    [2, 0, 0] -> do
      modifyIORef baseI (\i -> (i - 1 + nXs) `rem` nXs)
    [3, i, 0] -> do
      -- VU.unsafeFreeze xs >>= print
      j <- getIndex $ pred i
      x <- VUM.read xs j
      print x
    _ -> error "unreachable!"
